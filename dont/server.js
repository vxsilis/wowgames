...
//assing port
// const port = process.env.PORT || 37475;
...
io.on('connection', socket => {
    ...
    socket.on('gameInitialize', function(roomcode) {
        ...
        io.to(roomcode).emit('gameState', turn, move);

    })

    // emit gamestate to players of specific room
    socket.on('gameState', function(whoPlayed, move, roomcode) {

        io.to(roomcode).emit('gameState', whoPlayed, move);
